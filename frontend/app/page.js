...
export default function HomePage() {
  ...
  const canSubmit = useMemo(() => {
    return prompt.trim().length > 0 && !!styleFile && !isGenerating && !isCheckingFile && !error;
  }, [error, isCheckingFile, isGenerating, prompt, styleFile]);

  async function handleFileChange(event) {
    const file = event.target.files?.[0];
    setError(""); setClipDuration(null); setStyleFile(null);
    if (!file) return;
    if (!isSupportedAudioFile(file)) { setError("Upload a supported audio file: WAV, MP3, OGG, or FLAC."); return; }
    if (file.size > MAX_FILE_SIZE_MB * 1024 * 1024) { setError("Keep the reference clip under " + MAX_FILE_SIZE_MB + " MB."); return; }
    try {
      setIsCheckingFile(true);
      const duration = await getAudioDuration(file);
      if (Math.abs(duration - TARGET_CLIP_DURATION) > DURATION_TOLERANCE) {
        setError("The reference clip should be about " + TARGET_CLIP_DURATION + " seconds. Detected " + duration.toFixed(2) + " seconds.");
        return;
      }
      setStyleFile(file); setClipDuration(duration);
  ...
  async function handleSubmit(event) {
    ...
    if (!prompt.trim()) { setError("Write a prompt before generating."); return; }
    if (!styleFile) { setError("Upload a valid 5-second reference clip first."); return; }
    ...
    try {
      ...
      const response = await fetch(API_BASE_URL + "/generate/style", { method: "POST", body: formData });
      if (!response.ok) {
        let message = "Generation failed. Please try again.";
        const responseText = await response.text();
        try { const data = JSON.parse(responseText); if (data?.detail) message = data.detail; } catch { if (responseText) message = responseText; }
      ...
      <section className="workspace">
        <form className="glass-card form-card" onSubmit={handleSubmit}>
          <div className="section-heading">
            ...
            <p>This interface is ready to call the existing <code>/generate/style</code> backend endpoint.</p>
          </div>
          <label className="field-label" htmlFor="style-audio">Reference audio</label>
          <label className="upload-zone" htmlFor="style-audio">
            <input id="style-audio" type="file" accept=".wav,.mp3,.ogg,.flac,audio/*" onChange={handleFileChange} />
            <span className="upload-title">{isCheckingFile ? "Validating clip..." : "Drop your 5-second file here"}</span>
          ...
          <textarea
            id="prompt" className="prompt-input" rows="6"
        ...
        <div className="glass-card info-card">
          <div className="section-heading">
            ...
            <p>Generated music appears here as soon as the backend returns the WAV file.</p>
          </div>
          {resultUrl ? (
            ...
            <div className="placeholder-panel">
              ...
              <p>Upload the reference clip, write a prompt, and submit to see the produced audio here.</p>
            </div>
          )}
        </div>
      </section>
      ...
      {isGenerating ? (
        <div className="loading-overlay" role="status" aria-live="polite">
          <div className="loading-card">
            ...
            <p className="loading-text">This can take a little while depending on the model and hardware.</p>
